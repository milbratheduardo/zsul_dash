...
const ModalSumulaJogo = ({ isVisible, onClose, currentColor, timeCasa, timeFora, jogoId, campeonatoId, nomeTimeCasa, nomeTimeFora }) => {
    ...
    const fetchTimeCasa = async () => {
      try {
        ...
        if (data.status === 200) {
          ...
          const validAtletasDetails = atletasDetails.filter((atleta) => atleta && atleta.name);
  ...
  const handleSubmit = async (e) => {
    ...
    Promise.all(estatisticasPromessas)
      .then((results) => {
  ...
  return (
    ...
    >
      <div
        className='w-full sm:w-[400px] md:w-[500px] lg:w-[600px] flex flex-col'
          ...
          <form className='mt-4 space-y-4' onSubmit={handleSubmit}>
            ...
            <div className='-space-y-px'>
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className={`field-margin ${index !== 0 ? 'mt-2' : ''} ${field.type === 'dropdown' ? 'mb-2' : ''}`}
                >
                  {field.type === 'dropdown' ? (
                    <div>
                      <select
              ...
              {selects.map((select, index) => (
                <div key={select.id} className='flex items-center space-x-2 mt-2'>
                  ...
                  >
                    ...
                    {select.teamId === timeCasa &&
                      atletasTimeCasa.map((atleta) => (
                        <option key={atleta.elencoId} value={atleta.elencoId}>
                          {atleta.name}
                        </option>
                      ))}
                    {select.teamId === timeFora &&
                      atletasTimeFora.map((atleta) => (
                        <option key={atleta.elencoId} value={atleta.elencoId}>
                          {atleta.name}
                        </option>
                      ))}
