...
const SumulasDetalhes = () => {
  ...
  const handleStatusChange = async (atleta, newStatus) => {
    try {
      const responseGet = await fetch(` http://0.tcp.sa.ngrok.io:17723/sumula/elenco/${atleta._id}`);
      if (!responseGet.ok) {
      ...
      const responsePatch = await fetch(` http://0.tcp.sa.ngrok.io:17723/sumula/${idSumula}`, {
        method: 'PATCH',
        headers: {
  ...
  useEffect(() => {
    const fetchAtletasIds = async () => {
      try {
        const response = await fetch(` http://0.tcp.sa.ngrok.io:17723/sumula/team/${teamId}`);
        const data = await response.json();
      ...
      try {
        if (atletasIds && atletasIds.length > 0) {
          ...
          for (const elencoId of atletasIds) {
            const response = await fetch(` http://0.tcp.sa.ngrok.io:17723/elenco/${elencoId.elencoId}`);
            if (!response.ok) {
