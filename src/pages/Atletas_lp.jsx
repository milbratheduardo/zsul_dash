...
import { Header, Button, ModalAtleta, ModalAtletasOpcoes, Sidebar_lp, Navbar_lp, ModalAtletasOpcoes_lp } from '../components';
...
const Atletas_lp = () => {
  ...
  const fetchAtletas = async (name = '') => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}elenco/?name=${name}`);
      if (!response.ok) {
  ...
  const handleSearch = (args) => {
    if (args.requestType === 'searching') {
      const searchValue = args.searchString;
      setSearchTerm(searchValue);
      fetchAtletas(searchValue);
    }
  };
  ...
  const atletasGrid = [
    ...
    },
    {
      ...
      width: '150',
      textAlign: 'Center',
      template: (atleta) => (<a>Sub-{atleta.category}</a>),
    },
  ];
          ...
          {!showAtletasOpcoes && (
            <div className='m-2 md:m-10 p-2 md:p-10 bg-white rounded-3xl'>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Header category='Atletas' title={loading ? 'Carregando...' : 'Pesquise o Atleta Desejado'} />
              </div>
              <GridComponent
                dataSource={atletas}
                allowPaging
